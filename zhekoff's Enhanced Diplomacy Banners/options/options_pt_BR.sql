----------------------------------
-- Portuguese Localization for Enhanced Diplomacy Banners Settings (Credit to LeonardoWelter)
----------------------------------
INSERT OR REPLACE INTO LocalizedText
    (Tag, Language, Text)
VALUES

    -- Option Group
    ("LOC_OPTIONS_GROUP_ZHEKOFF_ENHANCED_DIPLO_BANNERS",
    "pt_BR",
    "Enhanced Diplomacy Banners"),

    -- Icon Style Option
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_ICON_STYLE_NAME",
    "pt_BR",
    "Estilo dos Ícones de Relacionamento"),
    
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_ICON_STYLE_DESCRIPTION",
    "pt_BR",
    "Escolha o estilo visual para os ícones de relacionamento exibidos nas faixas de diplomacia."),
    
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_ICON_STYLE_CLASSIC",
    "pt_BR",
    "Clássico"),
    
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_ICON_STYLE_EMOJI",
    "pt_BR",
    "Emoji"),

    -- Show Civilization Symbol Option
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_SHOW_CIV_SYMBOL_NAME",
    "pt_BR",
    "Mostrar Símbolo da Civilização"),
    
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_SHOW_CIV_SYMBOL_DESCRIPTION",
    "pt_BR",
    "Exibir símbolos de civilização nos banners diplomáticos."),
    
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_SHOW_CIV_SYMBOL_YES",
    "pt_BR",
    "Sim"),
    
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_SHOW_CIV_SYMBOL_NO",
    "pt_BR",
    "Não"),

    -- Always Show Extended Yields Option
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_ALWAYS_SHOW_EXTENDED_YIELDS_NAME",
    "pt_BR",
    "Sempre Mostrar Rendimentos Adicionais"),
    
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_ALWAYS_SHOW_EXTENDED_YIELDS_DESCRIPTION",
    "pt_BR",
    "Sempre exibir rendimentos adicionais sem necessidade de passar o mouse."),
    
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_ALWAYS_SHOW_EXTENDED_YIELDS_YES",
    "pt_BR",
    "Sim"),
    
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_ALWAYS_SHOW_EXTENDED_YIELDS_NO",
    "pt_BR",
    "Não"),

    -- Background Style Option
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_BACKGROUND_STYLE_NAME",
    "pt_BR",
    "Estilo de Fundo do Banner"),

    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_BACKGROUND_STYLE_DESCRIPTION",
    "pt_BR",
    "Escolha o estilo de fundo para os banners diplomáticos."),

    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_BACKGROUND_STYLE_DEFAULT",
    "pt_BR",
    "Padrão"),

    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_BACKGROUND_STYLE_ORIGINAL",
    "pt_BR",
    "Original"),

    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_BACKGROUND_STYLE_GOLDEN_HOUR",
    "pt_BR",
    "Ouro"),

    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_BACKGROUND_STYLE_FOREST_DEPTHS",
    "pt_BR",
    "Floresta"),

    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_BACKGROUND_STYLE_ROYAL_TWILIGHT",
    "pt_BR",
    "Fantasma"),

    -- Checkbox for 'Hide Banners'
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_HIDE_BANNERS_NAME",
    "pt_BR",
    "Ocultar Banners Diplomáticos"),

    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_HIDE_BANNERS_DESCRIPTION",
    "pt_BR",
    "Oculta os banners diplomáticos - eles só aparecerão ao passar o mouse."),

    -- Checkbox for disabling leader expressions
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_DISABLE_LEADER_EXPRESSIONS_NAME",
    "pt_BR",
    "Desativar Expressões de Líderes"),

    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_DISABLE_LEADER_EXPRESSIONS_DESCRIPTION",
    "pt_BR",
    "Desativa os retratos dinâmicos que mudam com base no seu relacionamento com aquele líder."),

    -- Compact Mode Option
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_COMPACT_MODE_NAME",
    "pt_BR",
    "Tamanho da Faixa"),
    
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_COMPACT_MODE_DESCRIPTION",
    "pt_BR",
    "Ajuste o tamanho dos banners diplomáticos para uma exibição mais compacta."),
    
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_COMPACT_MODE_NORMAL",
    "pt_BR",
    "Normal"),
    
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_COMPACT_MODE_COMPACT",
    "pt_BR",
    "Compacto"),
    
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_COMPACT_MODE_ULTRA_COMPACT",
    "pt_BR",
    "Ultra Compacto"),

    -- Show All Yields on Local Player Hover Option
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_SHOW_ALL_YIELDS_ON_LOCAL_HOVER_NAME",
    "pt_BR",
    "Mostrar Todos os Rendimentos ao Passar o Mouse Sobre o Jogador Local"),
    
    ("LOC_MOD_ZHEKOFF_DIPLO_RIBBON_SHOW_ALL_YIELDS_ON_LOCAL_HOVER_DESCRIPTION",
    "pt_BR",
    "Ao passar o mouse sobre o retrato do seu próprio líder, mostra também os rendimentos detalhados de todos os outros líderes.");
